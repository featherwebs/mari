...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__components_MapLocationSelector_vue__ = __webpack_require__(16);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__components_MapLocationSelector_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2__components_MapLocationSelector_vue__);
...
var defaults = { custom: {} };
post_type.alias.forEach(function (a) {
  defaults[a.slug] = a.default;
});
post_type.custom.forEach(function (a) {
...
var createMode = typeof post === 'undefined';
...
window.postapp = new Vue({
  ...
  mounted: function mounted() {
    ...
    if (this.post_type) {
      ...
      this.post_type_non_images.map(function (field) {
        if (_this.post.custom.length) {
          _this.post.custom.map(function (postCustom) {
            if (field.slug == postCustom.slug) customData.push(_extends({}, field, postCustom, { id: field.id, title: field.title })); // id to preserve custom_field id in case of post_type type
          });
        } else {
      ...
      this.post_type_images.map(function (field) {
        if (!_this.post.images.filter(function (c) {
          return c.slug == field.slug || c.pivot && c.pivot.slug == field.slug;
        }).length) {
          field.thumbnail = null;
          _this.post.images.push(Object.assign({}, field));
        }
      });
    }
  },
  ...
  watch: {
    'post.title': function postTitle(value) {
      if (createMode) this.post.slug = this.slugify(value);
    },
    'post.slug': function postSlug(value) {
      this.post.slug = this.slugify(value);
    },
    'post.post_type_id': function postPost_type_id(value) {
      this.post.post_type = this.post_types.find(function (pt) {
        return pt.id == value;
      });
    }
  },
  computed: {
    'post_type_images': function post_type_images() {
      if (this.post_type) return this.post_type.custom.filter(function (pt) {
        return pt.type == 'image' || pt.type == 'multiple-images';
      });
      return [];
    },
    'post_type_non_images': function post_type_non_images() {
      if (this.post_type) return this.post_type.custom.filter(function (pt) {
        return pt.type != 'image' && pt.type != 'multiple-images';
      });
      return [];
  ...
  methods: {
    ...
    removeImageField: function removeImageField(obj) {
      this.post.images = this.post.images.filter(function (i) {
        return i.id !== obj.id;
      });
    },
    ...
    addPostsRelation: function addPostsRelation(slug, id, multiple) {
      ...
      if (multiple) this.post.posts = [].concat(_toConsumableArray(this.post.posts), [_extends({}, relatedPost, { pivot: { slug: slug } })]);else this.post.posts = [].concat(_toConsumableArray(this.post.posts.filter(function (p) {
        return p.pivot.slug != slug;
      })), [_extends({}, relatedPost, { pivot: { slug: slug } })]);
    },
    removePostsRelation: function removePostsRelation(slug, id) {
      this.post.posts = this.post.posts.filter(function (p) {
  ...
  components: {
    ...
    MapLocationSelector: __WEBPACK_IMPORTED_MODULE_2__components_MapLocationSelector_vue___default.a
  }
});
